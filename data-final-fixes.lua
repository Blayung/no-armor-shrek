...
------------------------------------------------------------------------------------
local CHAR_NAME = SHREK.char_name

local mod_list = {
  ["RitnCharacters"] = "0.2.5",
}

SHREK.dwrite("mod_list: " .. serpent.block(mod_list))
-- Add any mod that may have registered itself to list.
for mod, version in pairs(SHREK_keep_default_character) do
...
SHREK.dwrite("new mod_list: " .. serpent.block(mod_list))
...
local function check_version(name, needed)
  ...
  -- Mod is active
  if found then
    SHREK.dwrite(string.format("Checking mod \"%s\": Found version %s, require version %s",
                              name, found, needed))
    ...
    if version[1] < needed[1] then
      SHREK.dwrite(string.format("Major number is too small: %g < %g", version[1], needed[1]))
      ret = false

    -- Major number is greater: Hit!
    elseif version[1] > needed[1] then
      SHREK.dwrite(string.format("Major number is greater: %g > %g", version[1], needed[1]))
      ret = true

    -- Major number is same, minor number is greater: Hit!
    elseif version[2] > needed[2] then
      SHREK.dwrite(string.format("Minor number is greater: %g > %g", version[2], needed[2]))
      ret = true
    ...
    elseif version[2] == needed[2] and version[3] >= needed[3] then
      SHREK.dwrite(string.format("Least number is greater or equal: %g >= %g", version[3], needed[3]))
      ret = true

    -- Version is smaller than required
    else
      SHREK.dwrite(string.format("Least number is too small: %g < %g", version[3], needed[3]))
      ret = false
    end

  -- Mod isn't active
  else
    SHREK.dwrite(string.format("Mod \"%s\" is not active!", name))
    ret = false
  end
...
for name, version in pairs(mod_list) do
  if check_version(name, version) then
    SHREK.dwrite(string.format("\"%s\" is active! Won't overwrite vanilla character and corpse.",
                              name))
...
for e, entities in ipairs({char, corpse}) do
  ...
  if replace_base_char then
    SHREK.dwrite(string.format("Stand-alone mode: Overwriting vanilla \"%s\" with \"%s\"",
  ...
  -- Keep base character/corpse
    ...
    SHREK.dwrite(string.format("Keeping both vanilla \"%s\" and \"%s\"!", entities[name].type, name))
    ...
    SHREK.dwrite(string.format("Added localization for \"%s\"!", name))

    -- Make sure the Shrek won't have the default character's corpse!
    if c then
      entities[name].character_corpse = name .. "-corpse"
      SHREK.dwrite(string.format("Activated new corpse for \"%s\": \"%s\"!",
